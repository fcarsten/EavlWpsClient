...
public class ConditionalProbabilityWpsClient extends EavlWpsClient {
    ...
    public final static String HPI_KDE_DPUT_SERVICE_ID = "org.n52.wps.server.r.hpikdedput";
    ...
    public HpiKde hpiKdeDput(double[][] gclr3, double[][] evalpts)
        ...
        return hpiKdeDput(gclr3[0].length, toWpsInputString(gclr3),
                evalpts[0].length, toWpsInputString(evalpts));
    }

    private HpiKde hpiKdeDput(int nGclr3Cols, String gclr3Str, int nEvalptsCols,
            String evalptsStr) throws IOException, WPSClientException {
        HashMap<String, Object> parameters = new HashMap<String, Object>();
        parameters.put("nGclr3Cols", "" + nGclr3Cols);
        parameters.put("gclr3Str", gclr3Str);
        ...
        ExecuteResponseAnalyser analyser = executeProcess(
                HPI_KDE_DPUT_SERVICE_ID, parameters);

        String resStr = WpsUtils.getResultString(analyser, "output");

//        Writer out = new BufferedWriter(new OutputStreamWriter(
//                new FileOutputStream("output.dput"), "UTF-8"));
//        out.write(resStr);
//        out.close();

        DputFactory df = new DputFactory();
        DputListStructure res = (DputListStructure) df.parse(new ByteArrayInputStream(resStr.getBytes(StandardCharsets.UTF_8)));
        return new HpiKde(res);
    }
